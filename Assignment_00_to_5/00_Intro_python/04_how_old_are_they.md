...
def main():
    anton : int = 3  # Anton's age is given as 21 years old
    beth : int = 4 + anton  # Beth is 6 years older than Anton, so add 6 to Anton's age to get Beth's
    chen : int = 5 + beth  # Chen is 20 years older than Beth, so add 20 to Beth's age to get Chen's
    drew  : int= 15 + anton  # Drew is as old as Chen's age plus Anton's age, so add them together
